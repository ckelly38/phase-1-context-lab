...
function createTimeInOrOutEvent(datestr, usetimein)
{
    ...
    if (usetimein)
    {
        this.timeInEvents.push(mytimeinobj);
        //console.log(this.timeInEvents[0].type);
        //console.log(this.timeInEvents[0].hour);
        //console.log(this.timeInEvents[0].date);
    }
    else
    {
        this.timeOutEvents.push(mytimeinobj);
        //console.log(this.timeOutEvents[0].type);
        //console.log(this.timeOutEvents[0].hour);
        //console.log(this.timeOutEvents[0].date);
    }
    return this;
}
function createTimeInEvent(datestr)
{
    return createTimeInOrOutEvent.call(this, datestr, true);
}
function createTimeOutEvent(datestr)
{
    return createTimeInOrOutEvent.call(this, datestr, false);
}

function noMissingPunchesFor()
{
    //console.log("this = " + this);
    //console.log("this.timeInEvents = " + this.timeInEvents);
    //console.log("this.timeOutEvents = " + this.timeOutEvents);
    //console.log("this.timeInEvents.length = " + this.timeInEvents.length);
    //console.log("this.timeOutEvents.length = " + this.timeOutEvents.length);
    if (this.timeInEvents.length == this.timeOutEvents.length);
    else
    {
        if (this.timeInEvents.length > this.timeOutEvents.length)
        {
...
function hoursWorkedOnDate(datestr)
{
    ...
    if (noMissingPunchesFor.call(this));
    ...
    let indates = this.timeInEvents.map((item) => getDateForItem(item));
    let outdates = this.timeOutEvents.map((item) => getDateForItem(item));
...
function testHoursWorkedOnDay()
{
    ...
    else
    {
        ...
        {
            createTimeInEvent.call(myemp, "2020-03-30 2015");
            if (testworkedcontinuously);
            else createTimeOutEvent.call(myemp, "2020-03-31 0100");
        }
        if (testworkedoneshift && !testworkedcontinuously && !testworkedinbriefly &&
            !testoneoutonly && !testoneinonly)
        {
            createTimeInEvent.call(myemp, "2020-03-31 0815");
            createTimeOutEvent.call(myemp, "2020-03-31 1200");
        }
        //else;//do nothing
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    createTimeInEvent.call(myemp, "2020-03-31 1230");
                    createTimeOutEvent.call(myemp, "2020-03-31 1700");
                }
                createTimeInEvent.call(myemp, "2020-03-31 2015");
            }
            createTimeOutEvent.call(myemp, "2020-04-01 0200");
        }
    }

    let myhrsemp = hoursWorkedOnDate.call(myemp, "2020-03-31");
    console.log("testnohours = " + testnohours);
    console.log("testworkedoneshift = " + testworkedoneshift);
    console.log("testworkedcontinuously = " + testworkedcontinuously);
    console.log("testworkedinbriefly = " + testworkedinbriefly);
...
function getAllValidDatesForEmp()
{
    if (noMissingPunchesFor.call(this));
    else throw "you missed some punches!";
    ...
    for (let n = 0; n < this.timeInEvents.length; n++)
    {
        let addit = true;
        ...
        else
        {
            ...
            {
                if (myvdatesarr[k] === this.timeInEvents[n].date)
                {
                    addit = false;
                    break;
                }
        ...
        if (addit) myvdatesarr.push(this.timeInEvents[n].date);
        //else;//do nothing
    }
    for (let n = 0; n < this.timeOutEvents.length; n++)
    {
        let addit = true;
        if (myvdatesarr.length < 1);
        else
        {
            for (let k = 0; k < myvdatesarr.length; k++)
            {
                if (myvdatesarr[k] === this.timeOutEvents[n].date)
                {
                    addit = false;
                    break;
                }
        ...
        if (addit) myvdatesarr.push(this.timeOutEvents[n].date);
        //else;//do nothing
    }
...
function calculatePayroll(emps)
{
    ...
    else
    {
        ...
        return emps.reduce((acc, emp) => acc + allWagesFor.call(emp), 0);
    }
}
...
function findEmployeeByFirstName(emps, firstname)
{
    ...
    if (emps == undefined || emps == null || emps.length < 1 ||
        firstname == undefined || firstname == null)
    {
        console.error("emps array was empty or null or undefined of firstname was undefined or null!");
        return null;
    }
    //else;//do nothing
    ...
    {
        if (emps[n].firstName === firstname) return emps[n];
        //else;//do nothing
    }
    console.error("no employee found with that first name!");
    return null;
}
